...
></a>
  ...
  methods: {
    async refreshData() {
      // axios
      //   .get(variables.API_URL + "Desserts/moldCake", this.config)
      //   .then((response) => {
      ...
      try {
        const response = await axios.get(
          variables.API_URL + "Desserts/moldCake"
        );
        console.log(response.data);
        this.dvm = response.data;
      } catch (error) {
        if (error.response && error.response.status === 401) {
          // await this.refreshToken(); // Refresh the token
          await this.refreshData(); // Retry the request with the new token
        } else {
    ...
    //       variables.API_URL + "Members/RefreshToken",
    //       {},
    //       this.config
    //     );
    //     const newToken = response.data;
    //     localStorage.setItem("jwtToken", newToken); // Update the stored token
