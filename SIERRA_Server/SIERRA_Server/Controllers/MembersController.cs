...
namespace SIERRA_Server.Controllers
{
    ...
    public class MembersController : ControllerBase
    {
        ...
        {
            if (_repo.IsEmailExist(email) == false) return BadRequest("請填入使用者名稱和密碼後，成為Sierra會員");
        ...
        public async Task<ActionResult<EditMemberDTO>> GetMember(int id)
        {
            ...
            if (member == null) return BadRequest("找不到該用戶");
        ...
        [HttpPut("EditMemberImage")]
        public async Task<ActionResult<EditMemberDTO>> EditMemberImage([FromForm] EditMemberImageDTO request)
        {
            var service = new MemberService(_repo);
            var result = await service.EditMemberImageAsync(request);
        ...
        [HttpGet("GetMemberImageUrl")]
        public async Task<ActionResult> GetMemberImageUrl(int id)
        {
            var service = new MemberService(_repo);
            var imageName = await service.GetMemberImageName(id);

            if (imageName == null) return BadRequest("找不到該用戶");

            var imageUrl = $"{Request.Scheme}://{Request.Host}/Uploads/{imageName}";
            return Ok(imageUrl);
        }
